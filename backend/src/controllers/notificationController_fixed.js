  ...
  try {
    const result = await db.query(
      `SELECT id, user_id, booking_id, type, title, message, channel, priority, 
              is_read, sent_at, created_at, metadata
       FROM Notifications 
       WHERE user_id = $1 
  ...
  try {
    const result = await db.query(
      `SELECT 
         ...
         COUNT(*) FILTER (WHERE priority = 'high')::integer as high_priority_count,
...
// Create notification (for internal use) - FIXED for INTEGER IDs
